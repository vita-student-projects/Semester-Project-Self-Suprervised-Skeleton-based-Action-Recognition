...
class TeacherStudentNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        studentdecode = self.decoder(studentpredict)
        studentproject = self.decoder_pred(studentdecode)
        
        studentpredict_ = studentpredict.reshape(B, -1, self.dim_feat)
        teacherdecoded_ = teacherencoded.reshape(B, -1, self.dim_feat)
        ...
        return studentencoded, teacherencoded, studentproject, x_patch, studentpredict, teachermasked, mask
