...
def get_raw_bodies_data(skes_path, ske_name, frames_drop_skes, frames_drop_logger):
    ...
    num_frames = int(str_data[0].strip('\r\n'))     # frame numbers. skeleton file中第一个数
    frames_drop = []
    bodies_data = dict()
    ...
    for f in range(num_frames):
        num_bodies = int(str_data[current_line].strip('\r\n'))      # 一般是1，如果对于这个frame没有数据，就是0
        current_line += 1

        if num_bodies == 0:  # no data in this frame, drop it
            frames_drop.append(f)  # 0-based index              将这个没有数据的frame的编号加到frames_drop中
            continue
        ...
        for b in range(num_bodies):
            ...
            for j in range(num_joints):
                temp_str = str_data[current_line].strip('\r\n').split()     # 读取第一行数据，其中0-2是joint的坐标，5-6是color数据
                joints[b, j, :] = np.array(temp_str[:3], dtype=np.float32)      # b一般是0，j是0-24
                colors[b, j, :] = np.array(temp_str[5:7], dtype=np.float32)     # b一般是0，j是0-24
                current_line += 1

            if bodyID not in bodies_data:  # Add a new body's data
                body_data = dict()
                body_data['joints'] = joints[b].reshape((1, 25, 3))  # ndarray: (25, 3)
                body_data['colors'] = colors[b].reshape((1, 25, 2))  # ndarray: (1, 25, 2)
                body_data['interval'] = [valid_frames]  # the index of the first valid frame
                ...
                body_data['joints'] = np.vstack((body_data['joints'], joints[b].reshape((1, 25, 3))))   # 在1上叠加
                body_data['colors'] = np.vstack((body_data['colors'], colors[b].reshape((1, 25, 2))))   # 在1上叠加
...
def get_raw_skes_data():
    # # save_path = '/home/ruihang/dataset/nturgb_d_skeletons_stats'
    # # skes_path = '/home/ruihang/dataset/nturgb+d_skeletons'
    ...
    for (idx, ske_name) in enumerate(skes_name):
        bodies_data = get_raw_bodies_data(skes_path, ske_name, frames_drop_skes, frames_drop_logger)    # {'name': ske_name, 'data': bodies_data, 'num_frames': num_frames - num_frames_drop}
...
if __name__ == '__main__':
    ...
    skes_path = '../../../../dataset/nturgb+d_skeletons/'
    stat_path = osp.join(save_path, 'statistics')
