...
def train_one_epoch(model: torch.nn.Module,
                    data_loader: Iterable, optimizer_student: torch.optim.Optimizer, optimizer_teacher: torch.optim.Optimizer,
    ...
    for data_iter_step, (samples, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if data_iter_step % accum_iter == 0:
            lr_sched.adjust_learning_rate(optimizer_student, data_iter_step / len(data_loader) + epoch, args)
            lr_sched.adjust_learning_rate(optimizer_teacher, data_iter_step / len(data_loader) + epoch, args)
        samples = samples.float().to(device, non_blocking=True)

        with torch.cuda.amp.autocast(enabled=args.enable_amp):
            studentencoded, teacherencoded, teacherproject, x_patch, studentpredict, teachermasked, mask = model(samples)
            ...
            loss_tea = decoder_loss(x_patch, teacherproject, mask)
