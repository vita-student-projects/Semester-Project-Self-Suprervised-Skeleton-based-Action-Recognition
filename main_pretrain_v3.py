...
from engine_pretrain_v3 import train_one_epoch
...
def get_args_parser():
    parser = argparse.ArgumentParser('SpatialFormer Pre-training', add_help=False)
    parser.add_argument('--config', default='./config/ntu60_xsub_pretrain_debug_v3.yaml', help='path to the configuration file')
...
def main(rank, world_size, args):
    ...
    if rank == 0:
        summary(model_without_ddp, [(8, 2, 100, 25, 3)])
        summary_info = summary(model_without_ddp, [(8, 2, 100, 25, 3)])
    ...
    parameters_to_update_stu = list(model_without_ddp.studentspatial.parameters()) + list(model_without_ddp.predictor.parameters())
    optimizer_student = torch.optim.AdamW(parameters_to_update_stu, lr=args.lr, betas=(0.9, 0.95))
    
    parameters_to_update_tea = list(model_without_ddp.decoder.parameters()) + list(model_without_ddp.decoder_pred.parameters())
    optimizer_teacher = torch.optim.AdamW(parameters_to_update_tea, lr=args.lr_decoder, betas=(0.9, 0.95))

    if global_rank == 0:
        print(optimizer_student)
        print(optimizer_teacher)

    loss_scaler = NativeScaler()

    # misc.load_model(args=args, model_without_ddp=model_without_ddp, optimizer=optimizer_student, loss_scaler=loss_scaler)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            optimizer_student, optimizer_teacher, device, epoch, loss_scaler,
            log_writer=log_writer,
            args=args
        )
        if args.output_dir and (epoch % 20 == 0 or epoch + 1 == args.epochs):
            misc.save_model_decoder(
                args=args, model=model, model_without_ddp=model_without_ddp, 
                optimizer_student=optimizer_student, optimizer_teacher=optimizer_teacher,
