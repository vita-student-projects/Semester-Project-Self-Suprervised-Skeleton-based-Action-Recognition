...
# DeiT: https://github.com/facebookresearch/deit
# --------------------------------------------------------
# pre-trained: 1, maxlen, 1, dim_feat
...
#         maxlen_checkpoint = pos_embed_checkpoint.shape[1]
#         maxlen_model = model.maxlen

#         if maxlen_model != maxlen_checkpoint:
#             print("Temporal position interpolate from %d to %d" % (maxlen_checkpoint, maxlen_model))
#             pos_tokens = pos_embed_checkpoint
#             pos_tokens = pos_tokens.permute(0, 3, 1, 2)
#             pos_tokens = torch.nn.functional.interpolate(
#                 pos_tokens, size=(maxlen_model, 1), mode='bicubic', align_corners=False)
...
def interpolate_temp_embed(model, checkpoint_model):
    if 'studentParallel.temp_embed' in checkpoint_model:
        ...
        if t_grid_size_model != t_grid_size_checkpoint:
            print("Temporal position interpolate from %d to %d" % (t_grid_size_checkpoint, t_grid_size_model))
            pos_tokens = pos_embed_checkpoint
            pos_tokens = pos_tokens.permute(0, 3, 1, 2)
            pos_tokens = torch.nn.functional.interpolate(
